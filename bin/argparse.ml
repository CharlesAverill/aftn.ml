...
let parse_arguments (game_data_path : string) : arguments =
  ...
  let speclist =
    ...
    ; ( "--select-options-tui"
      , Arg.Set select_options_tui
      , "Select arguments in the TUI rather than via command-line arguments" )
    ]
  in
  let usage_msg = "Usage: AFTN [OPTION...]" in
  ...
  ; print_map= !print_map
  ; log_level= !log_level
  ; select_options_tui= !select_options_tui }
